...
describe Label::DescriptionFormatter do
  ...
  describe '#format' do
    ...
    it 'returns the first sentence for long descriptions' do
...
DESC
      result = format other_long_description, '#'
    ...
    it 'returns the first sentence in multiple lines when this is too long' do
      unicorn_description = "\\Unicorn is an HTTP server for Rack applications "+
      ...
      result = format(unicorn_description, '  #')
      expect(result).to eq expected_description
    end
